#!/usr/bin/env python3
"""
r2bert_hybrid_cls_topk_cv.py

Hybrid R2BERT representation:
    r = concat( CLS , Top-K-avg-per-dim )
...
Optional:
    If --proj_dim 1024 is provided, the 1536-d concat vector is projected
    to 1024 before the final scoring head.
...
# -------------------------
# Model: BERT + [CLS || TopKAvgPerDim] + optional projection + linear head
# -------------------------
class R2BERTHybridCLSTopK(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        hybrid = torch.cat([cls_vec, topk_vec], dim=-1)  # (B,2H)
...
def main():
    ...
    # keep same output filenames so your summarize script won't break
